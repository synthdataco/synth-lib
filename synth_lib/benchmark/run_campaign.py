...
def setup_campaign(campaign_file: Path, repo_root: Path, campaigns_root: Path | None = None) -> CampaignState:
    ...
    build_snapshot(repo_root / "market_data", state.dir / "snapshot", cutoff=cfg.data_cutoff, start=cfg.data_start)
...
def main() -> None:
    ...
    if args.command == "setup":
        setup_campaign(args.campaign, args.repo_root)
        ...
        raise SystemExit(
            "verdict: run after the forward window — see synth_lib/benchmark/verdict/evaluate.py "
            "and the Task 14 runbook (generated by generate_cli.py then evaluate_candidate)."
        )
